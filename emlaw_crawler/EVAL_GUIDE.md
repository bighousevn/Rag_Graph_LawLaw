# Quy trình đánh giá emlaw_qa.json theo 4 tiêu chí

Dùng file này để chạy lại quy trình đánh giá chất lượng câu trả lời trong
`emlaw_qa.json` ở các phiên Claude Code sau. Vì tiêu chí 2 cần web search
thật (verify điều luật còn hiệu lực hay không), quy trình này luôn cần một
subagent Claude Code có quyền WebSearch/WebFetch trong vòng lặp — không có
cách nào chạy hoàn toàn bằng script độc lập nếu không có API key của một
LLM khác.
...
2. `trich_dung_dieu_luat` — **bắt buộc web search thật**, kiểm tra:
...
đặc biệt ghi rõ nếu văn bản bị thay thế thì thay thế bởi văn bản nào.
...
2. Với mỗi batch, spawn 1 agent `general-purpose` chạy song song
   ...
   - Đọc `emlaw_crawler/emlaw_qa.json`, chỉ xử lý đúng range `stt` được giao.
