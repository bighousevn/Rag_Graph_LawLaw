"""
Bước 5: Truy vấn đồ thị bằng các sub-triplet đã vector hoá (output/question_triplets_vectorized.json).
...
Input:  output/question_triplets_vectorized.json
        output/keyphrase_vectors.npy, output/keyphrase_rows.json
        output/kg_triplets.json
...
KEYPHRASE_VECTORS = os.path.join(OUTPUT_DIR, "keyphrase_vectors.npy")
KEYPHRASE_ROWS    = os.path.join(OUTPUT_DIR, "keyphrase_rows.json")
...
def load_index():
    ...
    edge_relation_names = {e["id"]: e["relation"] for e in kg["edges"]}
    ...
    return vectors, rows, edges, node_names, edge_relation_names, idxs_by_type, all_node_ids, all_edge_ids
...
def main():
    ...
    vectors, rows, edges, node_names, edge_relation_names, idxs_by_type, all_node_ids, all_edge_ids = load_index()
    log(f"Đã tải index: {len(rows)} keyphrase row, {len(edges)} edge")
