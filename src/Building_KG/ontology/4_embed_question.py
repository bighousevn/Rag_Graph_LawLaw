"""
Bước 4: Vector hoá các sub-triplet đã tách từ câu hỏi người dùng.

Bước tách câu hỏi tự do (văn nói) → sub-triplet (s, v, o) CHƯA làm ở đây — file input hiện tại
(`output/question_triplets.json`) là VÍ DỤ viết tay, đóng vai trò định nghĩa FORMAT mà bước tách
câu hỏi (sẽ làm sau, có thể bằng LLM giống src/Query/1_extract_and_nomalized.py cũ nhưng viết lại
theo domain giao thông) phải sinh ra.

Format input — 1 JSON array, mỗi phần tử là 1 sub-triplet của câu hỏi:
    [{"s": "người", "v": "uống", "o": "rượu bia"}, {"s": "người", "v": "lái", "o": "xe ô tô"}]
- Một câu hỏi có thể tách ra NHIỀU sub-triplet độc lập (xem Bước 5 — kết quả các sub-triplet sẽ
  được giao/hợp lại với nhau).
- Thành phần nào bị khuyết (câu hỏi không nêu, hoặc chính là ẩn số cần tìm) → dùng "*" thay vì bỏ
  trống. "*" nghĩa là "khớp với bất kỳ" ở đúng vị trí đó khi truy vấn (xử lý ở Bước 5).
